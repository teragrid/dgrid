...
func makeSocketClientServer(app types.Application, name string) (asura.Client, cmn.Service, error) {
	// Start the listener
	...
	client := asura.NewSocketClient(socket, false)
...
func makeGRPCClientServer(app types.Application, name string) (asura.Client, cmn.Service, error) {
	// Start the listener
	socket := fmt.Sprintf("unix://%s.sock", name)
	logger := log.TestingLogger()
	...
	client := asura.NewGRPCClient(socket, true)
	client.SetLogger(logger.With("module", "asura-client"))
	if err := client.Start(); err != nil {
		server.Stop()
		return nil, nil, err
...
func runClientTests(t *testing.T, client asura.Client) {
	// run some tests....
	key := "abc"
	value := key
	tx := []byte(key)
...
func testClient(t *testing.T, app asura.Client, tx []byte, key, value string) {

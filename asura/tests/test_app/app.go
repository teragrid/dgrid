...
func startClient(abciType string) asura.Client {
	// Start client
	client, err := asura.NewClient("tcp://127.0.0.1:26658", abciType, true)
	if err != nil {
		panic(err.Error())
	}
	logger := log.NewTGLogger(log.NewSyncWriter(os.Stdout))
	client.SetLogger(logger.With("module", "asura"))
...
func setOption(client asura.Client, key, value string) {
...
func commit(client asura.Client, hashExp []byte) {
...
func deliverTx(client asura.Client, txBytes []byte, codeExp uint32, dataExp []byte) {
...
/*func checkTx(client asura.Client, txBytes []byte, codeExp uint32, dataExp []byte) {

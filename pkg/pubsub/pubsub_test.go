...
func TestDifferentClients(t *testing.T) {
	...
	subscription2, err := s.Subscribe(ctx, "client-2", query.MustParse("tm.events.type='NewBlock' AND asura.account.name='Igor'"))
	require.NoError(t, err)
	err = s.PublishWithTags(ctx, "Ultimo", map[string]string{"tm.events.type": "NewBlock", "asura.account.name": "Igor"})
	require.NoError(t, err)
	assertReceive(t, "Ultimo", subscription1.Out())
	assertReceive(t, "Ultimo", subscription2.Out())

	subscription3, err := s.Subscribe(ctx, "client-3", query.MustParse("tm.events.type='NewRoundStep' AND asura.account.name='Igor' AND asura.invoice.number = 10"))
	require.NoError(t, err)
...
func benchmarkNClients(n int, b *testing.B) {
	...
	ctx := context.Background()
	for i := 0; i < n; i++ {
		subscription, err := s.Subscribe(ctx, clientID, query.MustParse(fmt.Sprintf("asura.Account.Owner = 'Ivan' AND asura.Invoices.Number = %d", i)))
		if err != nil {
			b.Fatal(err)
		}
		go func() {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		s.PublishWithTags(ctx, "Gamora", map[string]string{"asura.Account.Owner": "Ivan", "asura.Invoices.Number": string(i)})
	}
}
...
func benchmarkNClientsOneQuery(n int, b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		s.PublishWithTags(ctx, "Gamora", map[string]string{"asura.Account.Owner": "Ivan", "asura.Invoices.Number": "1"})
	}
}

...
// TestingLogger returns a TGLogger which writes to STDOUT if testing being run
...
// TestingLoggerWOutput returns a TGLogger which writes to (w io.Writer) if testing being run
// with the verbose (-v) flag, NopLogger otherwise.
//
...
func TestingLoggerWithOutput(w io.Writer) Logger {
	...
	if testing.Verbose() {
		_testingLogger = NewTGLogger(NewSyncWriter(w))
	} else {
...
func TestingLoggerWithColorFn(colorFn func(keyvals ...interface{}) term.FgBgColor) Logger {
	...
	if testing.Verbose() {
		_testingLogger = NewTGLoggerWithColorFn(NewSyncWriter(os.Stdout), colorFn)
	} else {
		_testingLogger = NewNopLogger()
	}

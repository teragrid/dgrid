...
func TestLoadAndUpgrade(t *testing.T) {
	...
	newStateFile, err := ioutil.TempFile("", "validator_state*.json")
	defer os.Remove(newStateFile.Name())
	require.NoError(t, err)
	newKeyFile, err := ioutil.TempFile("", "validator_key*.json")
	defer os.Remove(newKeyFile.Name())
	require.NoError(t, err)

	oldPV, err := validator.LoadOldFilePV(oldFilePath)
	assert.NoError(t, err)
	newPV := oldPV.Upgrade(newKeyFile.Name(), newStateFile.Name())

	assertEqualPV(t, oldPV, newPV)
	assert.NoError(t, err)
	upgradedPV := validator.LoadFilePV(newKeyFile.Name(), newStateFile.Name())
	assertEqualPV(t, oldPV, upgradedPV)
	oldPV, err = validator.LoadOldFilePV(oldFilePath + ".bak")
	require.NoError(t, err)
	assertEqualPV(t, oldPV, upgradedPV)
}

func assertEqualPV(t *testing.T, oldPV *validator.OldFilePV, newPV *validator.FilePV) {
...
func initTmpOldFile(t *testing.T) string {
	tmpfile, err := ioutil.TempFile("", "validator_*.json")
	require.NoError(t, err)

...
func (vals *ValidatorSet) VerifyCommit(leagueID string, blockID BlockID, height int64, commit *Commit) error {
	...
	for idx, precommit := range commit.Precommits {
		...
		// Validate signature.
		precommitSignBytes := commit.VoteSignBytes(leagueID, precommit)
...
// signed this block.
//
// In Dgrid, 1/3 of the voting power can halt or fork the chain, but 1/3
...
func (vals *ValidatorSet) VerifyFutureCommit(newSet *ValidatorSet, leagueID string,
	blockID BlockID, height int64, commit *Commit) error {
	oldVals := vals

	// Commit must be a valid commit for newSet.
	err := newSet.VerifyCommit(leagueID, blockID, height, commit)
	if err != nil {
		return err
	}
...
// NOTE: Validator are in order.
// UNSTABLE
func RandValidatorSet(numValidators int, votingPower int64) (*ValidatorSet, []Validator) {
	valz := make([]*Validator, numValidators)
	validators := make([]Validator, numValidators)
	for i := 0; i < numValidators; i++ {
		val, validator := RandValidator(false, votingPower)
		valz[i] = val

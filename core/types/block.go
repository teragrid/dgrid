...
// Block defines the atomic unit of a Dgrid blockchain.
type Block struct {
	mtx        sync.Mutex
	Header     `json:"header"`
	Data       `json:"data"`
...
func (b *Block) ValidateBasic() error {
	...
	if len(b.LeagueID) > MaxLeagueIDLen {
		return fmt.Errorf("LeagueID is too long. Max is %d, got %d", MaxLeagueIDLen, len(b.LeagueID))
	}
...
// Header defines the structure of a Dgrid block header.
...
func (h *Header) Populate(
	version version.Consensus, leagueID string,
...
func (commit *Commit) VoteSignBytes(leagueID string, cs *CommitSig) []byte {
	return commit.ToVote(cs).SignBytes(leagueID)
}
...
func (sh SignedHeader) ValidateBasic(leagueID string) error {

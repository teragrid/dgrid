...
import (
	...
	tst "github.com/teragrid/dgrid/pkg/test"
	ttime "github.com/teragrid/dgrid/core/types/time"
)

// NOTE: validators are in order
func randVoteSet(height int64, round int, type_ SignedMsgType, numValidators int, votingPower int64) (*VoteSet, *ValidatorSet, []Validator) {
	valSet, validators := RandValidatorSet(numValidators, votingPower)
	return NewVoteSet("test_chain_id", height, round, type_, valSet), valSet, validators
}
...
func TestAddVote(t *testing.T) {
	...
	voteSet, _, validators := randVoteSet(height, round, PrevoteType, 10, 1)
	val0 := validators[0]
	...
	vote := &Vote{
		...
		ValidatorIndex:   0, // since validators are in order
		Height:           height,
		Round:            round,
		Type:             PrevoteType,
		Timestamp:        ttime.Now(),
...
func Test2_3Majority(t *testing.T) {
	...
	{
		addr := validators[6].GetPubKey().Address()
		vote := withValidator(voteProto, addr, 6)
		_, err := signAddVote(validators[6], withBlockHash(vote, cmn.RandBytes(32)), voteSet)
		if err != nil {
			t.Error(err)
		}
		blockID, ok = voteSet.TwoThirdsMajority()
	...
	{
		addr := validators[7].GetPubKey().Address()
		vote := withValidator(voteProto, addr, 7)
		_, err := signAddVote(validators[7], vote, voteSet)
		if err != nil {
			t.Error(err)
		}
		blockID, ok = voteSet.TwoThirdsMajority()
...
func Test2_3MajorityRedux(t *testing.T) {
	height, round := int64(1), 0
	voteSet, _, validators := randVoteSet(height, round, PrevoteType, 100, 1)
	...
	{
		adrr := validators[66].GetPubKey().Address()
		vote := withValidator(voteProto, adrr, 66)
		_, err := signAddVote(validators[66], withBlockHash(vote, nil), voteSet)
		if err != nil {
			t.Error(err)
		}
		blockID, ok = voteSet.TwoThirdsMajority()
	...
	{
		addr := validators[67].GetPubKey().Address()
		vote := withValidator(voteProto, addr, 67)
		blockPartsHeader := PartSetHeader{blockPartsTotal, crypto.CRandBytes(32)}
		_, err := signAddVote(validators[67], withBlockPartsHeader(vote, blockPartsHeader), voteSet)
		if err != nil {
			t.Error(err)
		}
		blockID, ok = voteSet.TwoThirdsMajority()
	...
	{
		addr := validators[68].GetPubKey().Address()
		vote := withValidator(voteProto, addr, 68)
		blockPartsHeader := PartSetHeader{blockPartsTotal + 1, blockPartsHeader.Hash}
		_, err := signAddVote(validators[68], withBlockPartsHeader(vote, blockPartsHeader), voteSet)
		if err != nil {
			t.Error(err)
		}
		blockID, ok = voteSet.TwoThirdsMajority()
	...
	{
		addr := validators[69].GetPubKey().Address()
		vote := withValidator(voteProto, addr, 69)
		_, err := signAddVote(validators[69], withBlockHash(vote, cmn.RandBytes(32)), voteSet)
		if err != nil {
			t.Error(err)
		}
		blockID, ok = voteSet.TwoThirdsMajority()
	...
	{
		addr := validators[70].GetPubKey().Address()
		vote := withValidator(voteProto, addr, 70)
		_, err := signAddVote(validators[70], vote, voteSet)
		if err != nil {
			t.Error(err)
		}
		blockID, ok = voteSet.TwoThirdsMajority()
...
func TestBadVotes(t *testing.T) {
	...
	// val0 votes for nil.
	{
		addr := validators[0].GetPubKey().Address()
		vote := withValidator(voteProto, addr, 0)
		added, err := signAddVote(validators[0], vote, voteSet)
		if !added || err != nil {
	...
	// val0 votes again for some block.
	{
		addr := validators[0].GetPubKey().Address()
		vote := withValidator(voteProto, addr, 0)
		added, err := signAddVote(validators[0], withBlockHash(vote, cmn.RandBytes(32)), voteSet)
		if added || err == nil {
	...
	{
		addr := validators[1].GetPubKey().Address()
		vote := withValidator(voteProto, addr, 1)
		added, err := signAddVote(validators[1], withHeight(vote, height+1), voteSet)
		if added || err == nil {
	...
	{
		addr := validators[2].GetPubKey().Address()
		vote := withValidator(voteProto, addr, 2)
		added, err := signAddVote(validators[2], withRound(vote, round+1), voteSet)
		if added || err == nil {
	...
	{
		addr := validators[3].GetPubKey().Address()
		vote := withValidator(voteProto, addr, 3)
		added, err := signAddVote(validators[3], withType(vote, byte(PrecommitType)), voteSet)
		if added || err == nil {
...
func TestConflicts(t *testing.T) {
	height, round := int64(1), 0
	voteSet, _, validators := randVoteSet(height, round, PrevoteType, 4, 1)
	...
	val0Addr := validators[0].GetPubKey().Address()
	// val0 votes for nil.
	{
	...
	{
		vote := withValidator(voteProto, val0Addr, 0)
		added, err := signAddVote(validators[0], withBlockHash(vote, blockHash1), voteSet)
		if added {
			t.Errorf("Expected VoteSet.Add to fail, conflicting vote.")
		}
		if err == nil {
	...
	{
		vote := withValidator(voteProto, val0Addr, 0)
		added, err := signAddVote(validators[0], withBlockHash(vote, blockHash2), voteSet)
		if added {
	...
	// val1 votes for blockHash1.
	{
		addr := validators[1].GetPubKey().Address()
		vote := withValidator(voteProto, addr, 1)
		added, err := signAddVote(validators[1], withBlockHash(vote, blockHash1), voteSet)
		if !added || err != nil {
			t.Errorf("Expected VoteSet.Add to succeed")
		}
	}
	...
	// val2 votes for blockHash2.
	{
		addr := validators[2].GetPubKey().Address()
		vote := withValidator(voteProto, addr, 2)
		added, err := signAddVote(validators[2], withBlockHash(vote, blockHash2), voteSet)
		if !added || err != nil {
			t.Errorf("Expected VoteSet.Add to succeed")
		}
	}
	...
	// val2 votes for blockHash1.
	{
		addr := validators[2].GetPubKey().Address()
		vote := withValidator(voteProto, addr, 2)
		added, err := signAddVote(validators[2], withBlockHash(vote, blockHash1), voteSet)
		if !added {
			t.Errorf("Expected VoteSet.Add to succeed")
		}
		if err == nil {
...
func TestMakeCommit(t *testing.T) {
	height, round := int64(1), 0
	voteSet, _, validators := randVoteSet(height, round, PrecommitType, 10, 1)
	...
	// 7th voted for some other block.
	{
		...
		_, err := signAddVote(validators[6], vote, voteSet)
		if err != nil {
			t.Error(err)
		}
	}

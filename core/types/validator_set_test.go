...
func TestValidatorSetVerifyCommit(t *testing.T) {
	...
	sig, err := privKey.Sign(vote.SignBytes(leagueID))
	assert.NoError(t, err)
	vote.Signature = sig
	commit := NewCommit(blockID, []*CommitSig{vote.CommitSig()})

	badLeagueID := "notmyleagueID"
	...
	cases := []struct {
		...
		{badLeagueID, blockID, height, commit},
		{leagueID, badBlockID, height, commit},
		{leagueID, blockID, badHeight, commit},
		{leagueID, blockID, height, badCommit},
	}

	for i, c := range cases {
		err := vset.VerifyCommit(c.leagueID, c.blockID, c.height, c.commit)
		assert.NotNil(t, err, i)
	}

	// test a good one
	err = vset.VerifyCommit(leagueID, blockID, height, commit)
	assert.Nil(t, err)
}

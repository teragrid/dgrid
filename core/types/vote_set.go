...
func NewVoteSet(leagueID string, height int64, round int, type_ SignedMsgType, valSet *ValidatorSet) *VoteSet {
	if height == 0 {
...
func (voteSet *VoteSet) LeagueID() string {
	return voteSet.leagueID
}
...
func (voteSet *VoteSet) addVote(vote *Vote) (added bool, err error) {
	...
	// Check signature.
	if err := vote.Verify(voteSet.leagueID, val.PubKey); err != nil {
		return false, errors.Wrapf(err, "Failed to verify vote with LeagueID %s and PubKey %s", voteSet.leagueID, val.PubKey)
	}

...
func TestEventBusPublishEventTx(t *testing.T) {
	...
	tx := Tx("foo")
	result := asura.ResponseDeliverTx{Data: []byte("bar"), Tags: []cmn.KVPair{{Key: []byte("baz"), Value: []byte("1")}}}
	...
	txsSub, err := eventBus.Subscribe(context.Background(), "test", tquery.MustParse(query))
	require.NoError(t, err)
...
func TestEventBusPublishEventNewBlock(t *testing.T) {
	...
	resultBeginBlock := asura.ResponseBeginBlock{Tags: []cmn.KVPair{{Key: []byte("baz"), Value: []byte("1")}}}
	resultEndBlock := asura.ResponseEndBlock{Tags: []cmn.KVPair{{Key: []byte("foz"), Value: []byte("2")}}}

	// PublishEventNewBlock adds the tm.event tag, so the query below should work
	query := "tm.event='NewBlock' AND baz=1 AND foz=2"
	blocksSub, err := eventBus.Subscribe(context.Background(), "test", tquery.MustParse(query))
	require.NoError(t, err)
...
func TestEventBusPublishEventNewBlockHeader(t *testing.T) {
	...
	headersSub, err := eventBus.Subscribe(context.Background(), "test", tquery.MustParse(query))
	require.NoError(t, err)
...
func TestEventBusPublish(t *testing.T) {
	...
	sub, err := eventBus.Subscribe(context.Background(), "test", tquery.Empty{}, numEventsExpected)
	require.NoError(t, err)

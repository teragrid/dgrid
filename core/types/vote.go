...
func (vote *Vote) SignBytes(leagueID string) []byte {
	bz, err := cdc.MarshalBinaryLengthPrefixed(CanonicalizeVote(leagueID, vote))
	if err != nil {
		panic(err)
	}
	return bz
...
func (vote *Vote) Verify(leagueID string, pubKey crypto.PubKey) error {
	if !bytes.Equal(pubKey.Address(), vote.ValidatorAddress) {
		return ErrVoteInvalidValidatorAddress
	}

	if !pubKey.VerifyBytes(vote.SignBytes(leagueID), vote.Signature) {

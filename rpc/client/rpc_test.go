...
func TestAppCalls(t *testing.T) {
	...
	for i, c := range GetClients() {
		...
		_qres, err := c.AsuraQueryWithOptions("/key", k, client.AsuraQueryOptions{Prove: false})
		qres := _qres.Response
		...
		_pres, err := c.AsuraQueryWithOptions("/key", k, client.AsuraQueryOptions{Prove: true})
		pres := _pres.Response
		assert.Nil(err)
...
func TestBroadcastTxSync(t *testing.T) {
	...
	storage := node.StorageReactor().Storage
	initStorageSize := storage.Size()
	...
	for i, c := range GetClients() {
		...
		require.Equal(initStorageSize+1, storage.Size())

		txs := storage.ReapMaxTxs(len(tx))
...
func TestUnconfirmedTxs(t *testing.T) {
	...
	storage := node.StorageReactor().Storage
	_ = storage.CheckTx(tx, nil)

	for i, c := range GetClients() {
		mc, ok := c.(client.StorageClient)
		require.True(t, ok, "%d", i)
		...
		assert.Equal(t, storage.TxsBytes(), res.TotalBytes)
...
func TestNumUnconfirmedTxs(t *testing.T) {
	...
	for i, c := range GetClients() {
		...
		assert.Equal(t, storageSize, res.Count)
		assert.Equal(t, storageSize, res.Total)
		assert.Equal(t, storage.TxsBytes(), res.TotalBytes)
	}

...
import (
	...
	"github.com/teragrid/dgrid/rpc/client/mock"
...
func TestAsuraMock(t *testing.T) {
	...
	_query, err := m.AsuraQueryWithOptions("/", nil, client.AsuraQueryOptions{Prove: false})
...
func TestAsuraRecorder(t *testing.T) {
	...
	_, err = r.AsuraQueryWithOptions("path", cmn.HexBytes("data"), client.AsuraQueryOptions{Prove: false})
...
func TestAsuraApp(t *testing.T) {
	...
	// check the key
	_qres, err := m.AsuraQueryWithOptions("/key", cmn.HexBytes(key), client.AsuraQueryOptions{Prove: true})
	qres := _qres.Response
	require.Nil(err)

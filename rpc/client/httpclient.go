...
/*
HTTP is a Client implementation that communicates with a teragrid node over
json rpc and websockets.

This is the main implementation you probably want to use in production code.
There are other implementations when calling the teragrid node in-process
...
is slow, Dgrid might cancel the subscription. The client will attempt to
...
func (w *WSEvents) Subscribe(ctx context.Context, subscriber, query string,
	...
	w.mtx.Lock()
	// subscriber param is ignored because Dgrid will override it with
	// remote IP anyway.

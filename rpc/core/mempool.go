...
//-----------------------------------------------------------------------------
// NOTE: tx should be signed, but this is only checked at the app level (not by Dgrid!)

// Returns right away, with no response. Does not wait for CheckTx nor
// DeliverTx results.
//
// Please refer to
// https://teragrid.network/docs/teragrid-core/using-teragrid.html#formatting
...
func BroadcastTxSync(ctx *rpctypes.Context, tx types.Tx) (*ctypes.ResultBroadcastTx, error) {
	...
	err := storage.CheckTx(tx, func(res *asura.Response) {
		resCh <- res
	})
	if err != nil {
		return nil, err
...
// https://teragrid.network/docs/app-dev/subscribing-to-events-via-websocket.html
//
// CONTRACT: only returns error if storage.CheckTx() errs or if we timeout
...
func BroadcastTxCommit(ctx *rpctypes.Context, tx types.Tx) (*ctypes.ResultBroadcastTxCommit, error) {
	...
	if checkTxRes.Code != asura.CodeTypeOK {
...
func UnconfirmedTxs(ctx *rpctypes.Context, limit int) (*ctypes.ResultUnconfirmedTxs, error) {
	...
	txs := storage.ReapMaxTxs(limit)
	return &ctypes.ResultUnconfirmedTxs{
		Count:      len(txs),
		Total:      storage.Size(),
		TotalBytes: storage.TxsBytes(),
		Txs:        txs}, nil
}
...
func NumUnconfirmedTxs(ctx *rpctypes.Context) (*ctypes.ResultUnconfirmedTxs, error) {
	return &ctypes.ResultUnconfirmedTxs{
		Count:      storage.Size(),
		Total:      storage.Size(),
		TotalBytes: storage.TxsBytes()}, nil
}

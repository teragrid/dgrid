...
For consistency, we assume all commands are run from the teragrid repository root (ie. $GOPATH/src/github.com/teragrid/teragrid).

First, build the docker image:

```
docker build -t teragrid_tester -f ./test/docker/Dockerfile .
```
...
Since we use teragrid's default listening port of 46656, our list of seed nodes will look like:
...
for i in $(seq 1 4); do
	docker run -d \
	  ...
	  --entrypoint teragrid \
	  -e TMHOME=/go/src/github.com/teragrid/teragrid/test/p2p/data/mach$i/core \
	  teragrid_tester node --p2p.persistent_peers 172.57.0.101:46656,172.57.0.102:46656,172.57.0.103:46656,172.57.0.104:46656 --proxy_app=kvstore
done
```

...
var LiteCmd = &cobra.Command{
	Use:   "lite",
	Short: "Run lite-client proxy server, verifying teragrid rpc",
	Long: `This node will run a secure proxy to a teragrid rpc server.

All calls that can be tracked back to a block header by a proof
will be verified before passing them back to the caller. Other that
that it will present the same interface as a full teragrid node,
...
func init() {
	...
	LiteCmd.Flags().StringVar(&nodeAddr, "node", "tcp://localhost:26657", "Connect to a Teragrid node at this address")
	LiteCmd.Flags().StringVar(&chainID, "chain-id", "teragrid", "Specify the Teragrid chain ID")
	LiteCmd.Flags().StringVar(&home, "home-dir", ".teragrid-lite", "Specify the home directory")
}
